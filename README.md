...
/edit - изменить одну из последних 10 операций
/delete - удалить одну из последних 10 операций
...
- `/edit` показывает последние 10 операций. Администратор видит операции по всей таблице, обычный пользователь только свои.
- В `/edit` можно изменить сумму, описание, категорию, статус, валюту для наличных/безналичных и кошелек для крипты.
- `/delete` показывает обычному пользователю его последние 10 операций. После выбора бот отправляет запрос администраторам; запись удаляется только после одобрения. Администратор может удалять операции сразу.
- `/delete_last` по-прежнему удаляет последнюю запись пользователя после подтверждения.
- Если строка изменилась или была удалена между выбором и подтверждением, бот не удалит другую запись.
