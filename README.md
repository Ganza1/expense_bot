...
Автоматические отчеты и уведомления о новых оплатах отправляются всем chat_id из `ADMIN_CHAT_ID`. Несколько админов указываются через запятую или точку с запятой.
...
4. Если нужно, вставьте этот chat_id в `ADMIN_CHAT_ID`. Для нескольких админов используйте формат `111,222,333`.
5. Отправьте `/add`.
