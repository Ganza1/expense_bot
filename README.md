...
- Уведомление в `ADMIN_CHAT_ID` после создания новой оплаты
...
Автоматические отчеты и уведомления о новых оплатах отправляются в `ADMIN_CHAT_ID`.
...
Если `/history` вызывает пользователь из `ADMIN_CHAT_ID`, бот показывает последние 20 операций по всей таблице.
Если `/status` вызывает пользователь из `ADMIN_CHAT_ID`, бот позволяет менять статус последних 10 операций по всей таблице.
